  ...
  return <Dialog.Root open={open} onOpenChange={e => onOpenChange(e.open)} size={{ mdDown: 'full', md: 'xl' }}>
    <Portal><Dialog.Backdrop /><Dialog.Positioner><Dialog.Content>
      ...
      <Dialog.Body><Tabs.Root value={activeTab} onValueChange={event => setActiveTab(event.value as 'agent' | 'sms' | 'calls')}><Tabs.List maxW="full" overflowX="auto" overflowY="hidden" whiteSpace="nowrap"><Tabs.Trigger value="agent"><LuSettings2 /> Agente IA</Tabs.Trigger><Tabs.Trigger value="sms"><LuMessageSquare /> SMS</Tabs.Trigger><Tabs.Trigger value="calls"><LuPhoneCall /> Ligação IA</Tabs.Trigger></Tabs.List>
        ...
        <Tabs.Content value="sms"><Stack gap="4" pt="4"><Text fontSize="sm" color="fg.muted">Ao final de cada SMS, o CRM inclui automaticamente o link seguro da landing page, filtrado pelo CPF e pelo contrato do destinatário.</Text><Field.Root required><Field.Label>Título da campanha</Field.Label><Input value={title} onChange={e => setTitle(e.target.value)} /></Field.Root><Field.Root required><Field.Label>Mensagem</Field.Label><Textarea rows={4} maxLength={1400} value={message} onChange={e => setMessage(e.target.value)} /></Field.Root><ContractSelector contracts={eligible} withoutPhoneCount={withoutPhone.length} selected={selected} toggle={toggle} selectAll={() => setSelected(eligible.map(c => c.id))} clearSelection={() => setSelected([])} /><Button colorPalette="blue" loading={loading} onClick={() => dispatch('sms')}>Enviar SMS para {selected.length} contrato(s)</Button></Stack></Tabs.Content>

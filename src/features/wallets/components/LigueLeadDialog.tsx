...
function emailPreviewDocument(form: EmailTemplateForm) {
  const criteria = form.criteria;
  const replace = (value: string) => value.replaceAll('{{devedor_nome}}', 'Cliente de teste').replaceAll('{{nome_devedor}}', 'Cliente de teste').replaceAll('{{numero_contrato}}', 'CONTRATO-TESTE').replaceAll('{{contrato}}', 'CONTRATO-TESTE').replaceAll('{{credor_nome}}', 'Credor de teste').replaceAll('{{credor}}', 'Credor de teste').replaceAll('{{valor_oferta}}', 'R$ 100,00').replaceAll('{{botao_pagamento}}', '<div style="text-align:center;margin:24px 0"><span style="display:inline-block;background:#155dfc;color:#fff;padding:14px 22px;border-radius:8px;font-weight:700">Ver oferta e gerar Pix</span></div>');

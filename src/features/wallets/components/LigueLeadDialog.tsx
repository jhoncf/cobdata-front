...
function emailPreviewDocument(form: EmailTemplateForm) {
  ...
  // Keep the browser preview visually identical to the header rendered by the
  // sending service.  A full public URL is required because the preview runs
  // inside an iframe document (srcDoc), which has no reliable asset base URL.
  const header = criteria.includeCobcomHeader === false ? '' : '<tr><td style="padding:22px 32px;background:#0f4eea"><table role="presentation" cellspacing="0" cellpadding="0"><tr><td><img src="https://crm.maisqpago.com.br/cobcom-logo.png" width="52" height="52" alt="CobCom" style="display:block;border:0;object-fit:contain;background:#ffffff;border-radius:10px;padding:4px" /></td><td style="padding-left:14px;color:#ffffff;font:700 20px Arial,sans-serif">CobCom</td></tr></table></td></tr>';
...
function criteriaSummary(criteria: EmailTemplateCriteria) {
  ...
  if (criteria.paymentStatus === 'AGREEMENT_BREACHED') parts.push('Quebra de acordo');

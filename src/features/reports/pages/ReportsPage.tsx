import { Box, Button, Card, HStack, Input, SimpleGrid, Spinner, Table, Tabs, Text, VStack } from '@chakra-ui/react';
...
function Pagination({ report, onChange }: { report?: ListReport<unknown>; onChange: (page: number) => void }) {
  const meta = report?.meta;
  if (!meta || meta.totalPages <= 1) return null;
  return <HStack justify="space-between" mt="4" wrap="wrap"><Text fontSize="sm" color="fg.muted">Página {meta.page} de {meta.totalPages}</Text><HStack><Button size="sm" variant="outline" disabled={meta.page <= 1} onClick={() => onChange(meta.page - 1)}>Anterior</Button><Button size="sm" variant="outline" disabled={meta.page >= meta.totalPages} onClick={() => onChange(meta.page + 1)}>Próxima</Button></HStack></HStack>;
}

export default function ReportsPage() {
  const [period, setPeriod] = useState<ReportPeriod>(initialPeriod);
  const [page, setPage] = useState(1);
  const request = { ...period, page, limit: 50 };
  const agreements = useReportQuery<ListReport<SerasaAgreement>>('serasa-agreements', request);
  const pix = useReportQuery<ListReport<PixPayment>>('pix-payments', request);
  const communications = useReportQuery<ListReport<Communication>>('communications', request);
  const formatDate = (value: string) => new Date(value).toLocaleString('pt-BR');

  return <VStack align="stretch" gap="6">
    <PageHeader title="Relatórios" />
    <Card.Root><Card.Body><ReportHeader period={period} onChange={(nextPeriod) => { setPage(1); setPeriod(nextPeriod); }} /><Text mt="3" fontSize="sm" color="fg.muted">O período inicia, por padrão, nos últimos 30 dias.</Text></Card.Body></Card.Root>

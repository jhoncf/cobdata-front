...
import { downloadPixPayments, downloadSerasaAgreements, ReportPeriod, useReportFilters, useReportQuery } from '../api/useReportsQueries';

type Wallet = { name: string; creditor: { name: string } };
type SerasaAgreement = { id: string; contractNumber: string; debtorName: string; agreementReference: string; lastPaymentAt: string; agreementTotalAmount: number; totalInstallments: number | null; paidInstallments: number; wallet: Wallet };
type PixPayment = { amount: number; paidAt: string; externalPaymentId: string; paymentCharge: { attributedChannel: string | null } | null; contract: { id: string; contractNumber: string; debtorName: string; wallet: Wallet } };
...
function ReportHeader({ period, scope, onPeriodChange, onScopeChange }: { period: ReportPeriod; scope: Scope; onPeriodChange: (period: ReportPeriod) => void; onScopeChange: (scope: Scope) => void }) {
  const filters = useReportFilters();
  const wallets = (filters.data?.wallets ?? []).filter((wallet) => !scope.creditorId || wallet.creditorId === scope.creditorId);
  return <HStack gap="3" wrap="wrap" align="end">
    <Box><Text fontSize="sm" mb="1">Data inicial</Text><Input aria-label="Data inicial" type="date" value={period.startDate} onChange={(event) => onPeriodChange({ ...period, startDate: event.target.value })} /></Box>
    <Box><Text fontSize="sm" mb="1">Data final</Text><Input aria-label="Data final" type="date" value={period.endDate} onChange={(event) => onPeriodChange({ ...period, endDate: event.target.value })} /></Box>
    <Box><Text fontSize="sm" mb="1">Credor</Text><select aria-label="Filtrar por credor" value={scope.creditorId} style={{ height: '40px', minWidth: '220px', border: '1px solid', borderRadius: '6px', padding: '0 12px' }} onChange={(event) => onScopeChange({ creditorId: event.target.value, walletId: '' })}><option value="">Todos os credores</option>{filters.data?.creditors.map((creditor) => <option key={creditor.id} value={creditor.id}>{creditor.name}</option>)}</select></Box>
    <Box><Text fontSize="sm" mb="1">Carteira</Text><select aria-label="Filtrar por carteira" value={scope.walletId} style={{ height: '40px', minWidth: '220px', border: '1px solid', borderRadius: '6px', padding: '0 12px' }} onChange={(event) => onScopeChange({ ...scope, walletId: event.target.value })}><option value="">Todas as carteiras</option>{wallets.map((wallet) => <option key={wallet.id} value={wallet.id}>{wallet.name}</option>)}</select></Box>
  </HStack>;
}

function Summary({ total, amount, label }: { total: number; amount?: number; label: string }) { return <SimpleGrid columns={{ base: 1, sm: amount === undefined ? 1 : 2 }} gap="4" mb="5"><Box><Text fontSize="sm" color="fg.muted">{label}</Text><Text fontSize="2xl" fontWeight="bold">{total}</Text></Box>{amount !== undefined && <Box><Text fontSize="sm" color="fg.muted">Valor total</Text><Text fontSize="2xl" fontWeight="bold">{formatCurrency(amount)}</Text></Box>}</SimpleGrid>; }
function Pagination({ report, onChange }: { report?: ListReport<unknown>; onChange: (page: number) => void }) { const meta = report?.meta; if (!meta || meta.totalPages <= 1) return null; return <HStack justify="space-between" mt="4" wrap="wrap"><Text fontSize="sm" color="fg.muted">Página {meta.page} de {meta.totalPages}</Text><HStack><Button size="sm" variant="outline" disabled={meta.page <= 1} onClick={() => onChange(meta.page - 1)}>Anterior</Button><Button size="sm" variant="outline" disabled={meta.page >= meta.totalPages} onClick={() => onChange(meta.page + 1)}>Próxima</Button></HStack></HStack>; }

...
export default function ApiKeysPage() {
  ...
  return <Stack gap="6">
    ...
    {!isLoading && !keys?.length ? <EmptyState title="Nenhuma chave criada" description="Crie uma chave para liberar uma integração externa." /> :
      <Table.ScrollArea borderWidth="1px" rounded="md"><Table.Root size="sm"><Table.Header><Table.Row><Table.ColumnHeader>Nome</Table.ColumnHeader><Table.ColumnHeader>Credor autorizado</Table.ColumnHeader><Table.ColumnHeader>Token</Table.ColumnHeader><Table.ColumnHeader>Permissões</Table.ColumnHeader><Table.ColumnHeader>Último uso</Table.ColumnHeader><Table.ColumnHeader>Estado</Table.ColumnHeader><Table.ColumnHeader /></Table.Row></Table.Header><Table.Body>{keys?.map((key) => <Table.Row key={key.id}><Table.Cell>{key.name}</Table.Cell><Table.Cell>{key.accessAllCreditors ? 'Todos os credores da conta' : key.creditor ? <><Text>{key.creditor.name}</Text><Text fontSize="xs" color="fg.muted">{key.creditor.cnpj ?? 'CNPJ não informado'}</Text></> : 'Não vinculado (bloqueada)'}</Table.Cell><Table.Cell fontFamily="mono">{key.tokenPrefix}…</Table.Cell><Table.Cell><HStack gap="1" wrap="wrap">{key.scopes.map((scope) => <Badge key={scope} size="sm">{scope}</Badge>)}</HStack></Table.Cell><Table.Cell>{key.lastUsedAt ? new Date(key.lastUsedAt).toLocaleString('pt-BR') : 'Nunca'}</Table.Cell><Table.Cell><Badge colorPalette={key.revokedAt ? 'red' : 'green'}>{key.revokedAt ? 'Revogada' : 'Ativa'}</Badge></Table.Cell><Table.Cell>{!key.revokedAt && <HStack gap="1"><Button size="xs" variant="ghost" onClick={() => openEdit(key)} aria-label="Editar permissões"><LuPencil /></Button><Button size="xs" variant="ghost" colorPalette="red" onClick={() => setRevokeTarget(key)} aria-label="Revogar chave"><LuTrash2 /></Button></HStack>}</Table.Cell></Table.Row>)}</Table.Body></Table.Root></Table.ScrollArea>}
